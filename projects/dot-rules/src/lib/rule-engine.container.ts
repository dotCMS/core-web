...
export class RuleEngineContainer implements OnDestroy {
  ...
  onUpdateExpandedState(event: RuleActionEvent): void {
    ...
    if (rule._expanded) {
      ...
      if (rule._conditionGroups.length === 0) {
        const obs: Observable<ConditionGroupModel[]> = this._conditionGroupService.allAsArray(
          rule.key,
          Object.keys(rule.conditionGroups)
        );
        obs2 = obs.pipe(mergeMap((groups: ConditionGroupModel[]) => observableFrom(groups)));
      } else {
      ...
      if (rule._ruleActions.length === 0) {
        this._ruleActionService
          .allAsArray(rule.key, Object.keys(rule.ruleActions), this._ruleService._ruleActionTypes)
          .subscribe(actions => {
  ...
  onCreateRuleAction(event: RuleActionActionEvent): void {
    ...
    const rule = event.payload.rule;
    const priority = rule._ruleActions.length ? rule._ruleActions[rule._ruleActions.length - 1].priority + 1 : 1;
  ...
  onUpdateRuleActionType(event: RuleActionActionEvent): void {
    ...
    try {
      ...
      const type: ServerSideTypeModel = this._ruleService._ruleActionTypes[<string>event.payload.value];
    ...
    if (condition.isPersisted()) {
      this._conditionService.remove(condition).subscribe(result => {
        ...
        if (rule._conditionGroups.length === 0) {
          ...
          const conditionGroup = new ConditionGroupModel({ operator: 'AND', priority: 1 });
  ...
  private _handle403Error(e: CwError): boolean {
    ...
    try {
      if (e && e.response.status === HttpCode.FORBIDDEN) {
        const errorJson = e.response.body;
