...
fdescribe('ValidPublicAuthGuardService', () => {
    ...
    it('should redirect to to Main Portlet if User is logged in', fakeAsync(() => {
        let result: boolean;
        spyOnProperty(loginService, 'isLogin$', 'get').and.returnValue(Observable.of(true));
        publicAuthGuardService.canActivate(mockActivatedRouteSnapshot, mockRouterStateSnapshot)
    ...
    it('should allow access to the requested route if User is NOT logged in', fakeAsync(() => {
        let result: boolean;
