...
describe('ValidAuthGuardService', () => {
    ...
    it('should allow access to the requested route, User is logged in', fakeAsync(() => {
        let result: boolean;
        spyOnProperty(loginService, 'isLogin$', 'get').and.returnValue(Observable.of(true));
        authGuardService.canActivate(mockActivatedRouteSnapshot, mockRouterStateSnapshot)
            .subscribe(res => result = res);
        tick();
        expect(result).toBe(true);
    }));

    it('should denied access to the requested route, User is NOT logged in', fakeAsync(() => {
        let result: boolean;
