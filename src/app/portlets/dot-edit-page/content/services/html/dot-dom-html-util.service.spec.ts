...
describe('DotDOMHtmlUtilService', () => {
    ...
    it('should get a button html code', () => {
        ...
        const cssStyles = 'font-size: 0px;';
        const styleFunction = `onmouseover="this.style.backgroundColor='#e7e7e7'";`;
        const dataset = {
            a: 'a value',
            b: 'b value'
        };
        const buttonHTML = dotDOMHtmlUtilService.getButtomHTML(label, className, cssStyles, styleFunction, dataset);
        ...
        expect(button.style.cssText).toEqual(cssStyles);
        expect(button.onmouseover.toString()).toContain(`this.style.backgroundColor='#e7e7e7'`);
